...
def process_uploaded_file(filepath: str) -> List[Dict[str, Any]]:
    """Processes an uploaded file (PDF, Word, Excel, CSV) and returns indexed data (not added to global index here)."""
    indexed_data = []
        ...
        try:
            ...
            for i, page_obj in enumerate(reader.pages):
                ...
                if page_content and not page_content.isspace():
                    cleaned_page_text = re.sub(r'\bPage\s*\d+\s*\b', '', page_content, flags=re.IGNORECASE)
                    cleaned_page_text = re.sub(r'\r\n', '\n', cleaned_page_text)
                    cleaned_page_text = re.sub(r'(?<=\S)\s*\n\s*(?=\S\S)', '\n\n', cleaned_page_text)
                    cleaned_page_text = re.sub(r'\n{3,}', '\n\n', cleaned_page_text)
                    page_texts_parts.append(f"[File: {filename} - Page {i+1}]\n{cleaned_page_text.strip()}")
                ...
                for i, chunk in enumerate(chunks):
                    ...
                    if embedding and any(e != 0.0 for e in embedding):
                        indexed_data.append({
                            'text': chunk,
                            'embedding': embedding,
                            # Standardize source for retrieval logic
                            'source': f"Uploaded PDF: {filename} Chunk {i+1}"
                        })
            else:
                print(f"No text content extracted from uploaded PDF: {filename}")
        except Exception as e:
            print(f"Error processing uploaded PDF {filename}: {e}")

    elif file_extension == "docx":
        print(f"Processing uploaded Word document: {filename}")
        try:
                ...
                for i, chunk in enumerate(chunks):
                    ...
                    if embedding and any(e != 0.0 for e in embedding):
                        indexed_data.append({
                            'text': chunk,
                            'embedding': embedding,
                            'source': f"Uploaded Word: {filename} Chunk {i+1}"
                        })
            else:
                print(f"No text content extracted from uploaded Word doc: {filename}")
        except Exception as e:
            print(f"Error processing uploaded Word document {filename}: {e}")

    elif file_extension == "xlsx":
        print(f"Processing uploaded Excel file: {filename}")
        try:
            workbook = openpyxl.load_workbook(filepath, data_only=True)
            for sheet_name in workbook.sheetnames:
                worksheet = workbook[sheet_name]
                for r_idx, row in enumerate(worksheet.iter_rows(values_only=True)):
                    row_text_parts = [str(cell).strip() for cell in row if cell is not None and str(cell).strip()]
                    if row_text_parts:
                        ...
                        if embedding and any(e != 0.0 for e in embedding):
                            indexed_data.append({
                                'text': row_text,
                                'embedding': embedding,
                                'source': f"Uploaded Excel: {filename} Sheet: {sheet_name} Row: {r_idx+1}"
                            })
        except Exception as e:
            print(f"Error processing uploaded Excel file {filename}: {e}")
            ...
            for index, row in df.iterrows():
                ...
                if embedding and any(e != 0.0 for e in embedding):
                    indexed_data.append({
                        'text': text_chunk,
                        'embedding': embedding,
                        'source': f"Uploaded CSV: {filename} Row: {index+1}"
                    })
        except Exception as e:
            print(f"Error processing uploaded CSV file {filename}: {e}")

    else:
        print(f"Unsupported file type for upload: {file_extension}")

    print(f"Processed {len(indexed_data)} items from uploaded file: {filename}")

    # --- Add to global index and persist ---
    global in_memory_index
    if indexed_data:
        in_memory_index.extend(indexed_data)
        save_index_to_file(in_memory_index)
